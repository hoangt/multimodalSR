...
def main():
    ...
    if viseme:
        database_binaryDir = rootDir + 'database_binaryViseme'
    else:
    ...
    if not os.path.exists(pkl_path):
        ...
        processdataset_images(data_path=database_binaryDir, store_path=pkl_path, trainFraction=0.8, validFraction=0.1,
                              testFraction=0.1,
    ...
    if network_type == "google":
        cnnDict, l_out = buildNetworks.build_network_google(activation, alpha, epsilon, inputs,
                                                            nbClasses)  # 7.176.231 params
    elif network_type == "cifar10":
        cnn, l_out = buildNetworks.build_network_cifar10(activation, alpha, epsilon, inputs,
                                                         nbClasses)  # 9.074.087 params,    # with 2x FC1024: 23.634.855
